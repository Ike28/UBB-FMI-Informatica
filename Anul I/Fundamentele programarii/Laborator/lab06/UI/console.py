...
from Logic.undo import register_undo, remove_last_in_undo, get_undo_tree
from Logic.redo import register_redo, remove_last_in_redo, get_redo_tree, clear_redo_tree
...
def run_crud(thing_list):
    ...
    def handle_add(thing_list1):
        ...
        try:
            ...
            register_undo('adauga', [object_id], thing_list)
    ...
    def handle_modify(thing_list1):
        ...
        try:
            ...
            register_undo('modifica', [updated_thing], thing_list)
    ...
    def handle_delete(thing_list1):
        ...
        try:
            ...
            register_undo('sterge', [del_id], thing_list)
...
def run_operations(thing_list):
    ...
    def handle_move(thing_list1):
        ...
        try:
            ...
            register_undo('mutare', [source_room,destination_room], thing_list1)
    ...
    def handle_concat(thing_list1):
        ...
        try:
            ...
            register_undo('concat', [added_string, search_price], thing_list1)
    ...
    def handle_sort_by_price(thing_list1):
        ...
        register_undo('ordonare', [thing_list1], thing_list1)
        thing_list1 = sort_inventory_by_price(thing_list1)
        #if strict_equal(thing_list1, sorted_list):
            #print(f'\n{Colors.YELLOW}Inventarul este deja ordonat dupa acest criteriu.{Colors.END}')
        #else:
...
def run_console(thing_list):
    ...
    def handle_undo(thing_list1):
        undo_tree = get_undo_tree()
        try:
            if len(undo_tree) == 0:
                raise TypeError('\nNu exista operatii anterioare.')
            register_redo(undo_tree[len(undo_tree)-1], thing_list1)
            thing_list1 = handle_command(thing_list1, undo_tree[len(undo_tree)-1])
            remove_last_in_undo()
    ...
    def handle_redo(thing_list1):
        redo_tree = get_redo_tree()
        try:
            if len(redo_tree) == 0:
                raise TypeError('\nNu exista operatii ulterioare')
            redo_command = redo_tree[len(redo_tree)-1]
            register_undo(redo_command[0], redo_command[1:], thing_list)
            thing_list1 = handle_command(thing_list1, redo_command)
            remove_last_in_redo()
