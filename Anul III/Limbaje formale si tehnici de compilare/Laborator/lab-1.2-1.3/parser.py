...
class Parser:
    def __init__(self):
        self.__path_atomi = './resurse/atomi.in'
        self.__path_fip = './resurse/fip.out'
        self.__path_ts_constant = './resurse/ts_const.out'
        self.__path_ts_identificator = './resurse/ts_ident.out'
    ...
    def scan_program(self, path_fisier):
        ...
        for numar_linie, linie in linii:
            linie = linie.strip()  # elimina spatiile la inceput si la sfarsit
            try:
                self.scan_linie(linie)
            except (KeyError, ValueError) as err:
                # daca se ajunge aici => eroare lexicala
                # creeaza un mesaj de eroare si afiseaza
    ...
    @staticmethod
    def __e_identificator(cuvant):
        """
        Verifica daca un cuvant e identificator (utilizand regulile definite in program)
        """
        match = re.match('^([a-zA-Z])([a-zA-Z]|[0-9]|_){0,6}([a-zA-Z]|[0-9])*$', cuvant)
        if match and len(cuvant) < 256:
            return match
        else:
            return None

    @staticmethod
    def __e_constanta_intreaga(cuvant):
        """
        Verifica daca un cuvant e constanta intreaga (utilizand regulile definite in program)
        """
        return re.match('^(-)?([1-9]+[0-9]*)$|^0$', cuvant)

    @staticmethod
    def __e_constanta_hexa(cuvant):
        """
        Verifica daca un cuvant e constanta hexazecimala (utilizand regulile definite in program)
        """
        return re.match('^H[0-9A-F]+$', cuvant)

    @staticmethod
    def __e_constanta_reala(cuvant):
        """
        Verifica daca un cuvant e constanta reala (utilizand regulile definite in program)
        """
        return re.match('^(-)?[0-9]+\\.[0-9]*[1-9]$', cuvant)

    def __e_constanta_caracter(self, cuvant):
        """
        Verifica daca un cuvant e constanta caracter (utilizand regulile definite in program)
        """
        if len(cuvant) != 3 or cuvant[0] != "'" or cuvant[-1] != "'":
            return False
        regex_pattern = r'^[a-zA-Z]$|^[0-9]$|^' + self.__get_regex_splitter() + r'$'
        return re.match(regex_pattern, cuvant[1])

    @staticmethod
    def __e_constanta_string(cuvant):
        """
        Verifica daca un cuvant e constanta string (utilizand regulile definite in program)
        """
        if len(cuvant) < 3 or cuvant[0] != '"' or cuvant[-1] != '"':
            return False
        regex_pattern = r'^.+$'
        return re.match(regex_pattern, cuvant[1:-1])
    ...
    def __e_constanta(self, cuvant):
        ...
        """
        return self.__e_constanta_intreaga(cuvant) \
            or self.__e_constanta_caracter(cuvant) \
            or self.__e_constanta_string(cuvant) \
            or self.__e_constanta_reala(cuvant) \
            or self.__e_constanta_hexa(cuvant)

    def __e_operator(self, cuvant):
        """
        Verifica daca un cuvant e operator.
        """
        return cuvant in self.__operatori
    ...
    def __reconstruire_numere_cu_semn(self, lista_atomi):
        ...
        while idx < len(lista_atomi) - 1:
            ...
            e_int_sau_id_prev = self.__e_constanta_intreaga(prev_elem) or self.__e_identificator(prev_elem)
            e_int_sau_id_next = self.__e_constanta_intreaga(next_elem) or self.__e_identificator(next_elem)
            ...
            idx += 1
        e_int_sau_id_stl = self.__e_constanta_intreaga(lista_atomi[-3]) or self.__e_identificator(lista_atomi[-3])
        e_int_sau_id_last = self.__e_constanta_intreaga(lista_atomi[-1]) or self.__e_identificator(lista_atomi[-1])
    ...
    def __get_regex_splitter(self):
        ...
        splitter_string = r"(\s+)|"
        caractere_speciale = ['*', '+', '(', ')', '[', ']', '^', '$', '+=', '*=', '^=', '\'']
        for splitter_caracter in self.__operatori + self.__separatori:
    ...
    def __citire_atomi(self):
        """
        Citeste operatorii, separatorii si cuvintele rezervate din atomi.in
        """
        ...
        for linie_num, linie in linii:
            ...
            if tip_simboluri == 'operatori':
                self.__operatori = simboluri
            elif tip_simboluri == 'separatori':
...
parser = Parser()
fip, ts_const, ts_ident, mesaj = parser.scan_program("./programe/cmmdc.bas")
